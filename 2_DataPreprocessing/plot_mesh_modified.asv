function [ WarpedImage] = plot_mesh_modified(vertex,face,options)

% This function is modified by Rasoul Sharifian to texturize a flattned
% mesh. 20-02-2022
...
if not(isempty(texture))
    ...
    opts.EdgeColor = 'none';
    %Color
    texture1(:,:,1) = texture(:,:,1)';
    texture1(:,:,2) = texture(:,:,2)';
    texture1(:,:,3) = texture(:,:,3)';
%Gray
% texture1 = texture;
    patcht(face,vertex,face,texture_coords,texture1,opts);
    if size(texture1,3)==1
        colormap gray(256);
    else
        colormap jet(256);
    end
    ...
    WarpedImage = displayedImage.cdata;
